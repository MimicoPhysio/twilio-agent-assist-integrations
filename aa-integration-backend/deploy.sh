...
export ADMIN_ACCOUNT=${ADMIN_ACCOUNT:=$(gcloud config get-value account)}
...
export CONNECTOR_SERVICE_ACCOUNT_NAME='aa-ui-connector'
export INTERCEPTOR_SERVICE_ACCOUNT_NAME='aa-pubsub-interceptor'
...
if gcloud iam service-accounts describe "$connector_service_account" >/dev/null 2>&1; then
...
if gcloud iam service-accounts describe "$interceptor_service_account" >/dev/null 2>&1; then
...
if gcloud secrets describe "$JWT_SECRET_NAME" >/dev/null 2>&1; then
  echo "Updating existing secret $JWT_SECRET_NAME."
  printf "%s" "$JWT_SECRET_KEY" | gcloud secrets versions add "$JWT_SECRET_NAME" --data-file=-
else
...
if gcloud redis instances describe "$REDIS_INSTANCE_ID" --region="$SERVICE_REGION" >/dev/null 2>&1; then
...
elif gcloud compute networks vpc-access connectors describe "$VPC_CONNECTOR_NAME" --region="$SERVICE_REGION" >/dev/null 2>&1; then
...
# Build the environment variables string based on the selected auth option.
env_vars="REDISHOST=$REDIS_HOST,REDISPORT=$REDIS_PORT,GCP_PROJECT_ID=$GCP_PROJECT_ID,AUTH_OPTION=$AUTH_OPTION,APP_AUTH_OPTION=$APP_AUTH_OPTION"
if [ "$AUTH_OPTION" = "SalesforceLWC" ] || [ "$AUTH_OPTION" = "Salesforce" ]; then
    env_vars="$env_vars,SALESFORCE_DOMAIN=$SALESFORCE_DOMAIN,SALESFORCE_ORGANIZATION_ID=$SALESFORCE_ORGANIZATION_ID"
fi
if [ "$AUTH_OPTION" = "GenesysCloud" ]; then
    env_vars="$env_vars,GENESYS_CLOUD_ENVIRONMENT=$GENESYS_CLOUD_ENVIRONMENT"
fi
if [ "$AUTH_OPTION" = "Twilio" ] || [ "$APP_AUTH_OPTION" = "Twilio" ]; then
    env_vars="$env_vars,TWILIO_ACCOUNT_SID=$TWILIO_ACCOUNT_SID"
fi
gcloud_run_deploy_command+=("--set-env-vars" "$env_vars")

if [[ -z $VPC_CONNECTOR_NAME ]]; then
  echo "Deploying with Direct Egress when a Serverless VPC Access connector is not configured. "
  gcloud_run_deploy_command+=("--network" "$VPC_NETWORK" "--subnet" "$VPC_SUBNET" "--clear-vpc-connector")
else
  echo "Deploying with a Serverless VPC Access connector."
  gcloud_run_deploy_command+=("--vpc-connector" "$VPC_CONNECTOR_NAME" "--clear-network")
fi
"${gcloud_run_deploy_command[@]}"
...
gcloud_run_deploy_interceptor_command=(
...
if [[ -z $VPC_CONNECTOR_NAME ]]; then
  echo "Deploying with Direct Egress when a Serverless VPC Access connector is not configured."
  gcloud_run_deploy_interceptor_command+=("--network" "$VPC_NETWORK" "--subnet" "$VPC_SUBNET" "--clear-vpc-connector")
else
  echo "Deploying with a Serverless VPC Access connector."
  gcloud_run_deploy_interceptor_command+=("--vpc-connector" "$VPC_CONNECTOR_NAME" "--clear-network")
fi
"${gcloud_run_deploy_interceptor_command[@]}"

echo -e '\n\n ================= Create Cloud PubSub Topic ================== \n\n'

if gcloud pubsub topics describe "$CONVERSATION_LIFECYCLE_NOTIFICATIONS_TOPIC_ID" >/dev/null 2>&1; then
...
if gcloud pubsub topics describe "$AGENT_ASSIST_NOTIFICATIONS_TOPIC_ID" >/dev/null 2>&1; then
...
if gcloud pubsub topics describe "$NEW_MESSAGE_NOTIFICATIONS_TOPIC_ID" >/dev/null 2>&1; then
...
if gcloud pubsub topics describe "$NEW_RECOGNITION_RESULT_NOTIFICATION_TOPIC_ID" >/dev/null 2>&1; then
...
if gcloud iam service-accounts describe "$pubsub_service_account" >/dev/null 2>&1; then
...
# Get the URL of the deployed Cloud PubSub Interceptor Service.
interceptor_service_url=$(gcloud run services describe $INTERCEPTOR_SERVICE_NAME --platform managed --region $SERVICE_REGION --format 'value(status.url)')

# Create a subscription for the Pub/Sub topic you configured for new suggestions.
if gcloud pubsub subscriptions describe "$AGENT_ASSIST_NOTIFICATIONS_SUBSCRIPTION_ID" >/dev/null 2>&1; then
  echo "Updating subscription $AGENT_ASSIST_NOTIFICATIONS_SUBSCRIPTION_ID."
  ...
  echo "Creating subscription $AGENT_ASSIST_NOTIFICATIONS_SUBSCRIPTION_ID."
...
if gcloud pubsub subscriptions describe "$NEW_MESSAGE_NOTIFICATIONS_SUBSCRIPTION_ID" >/dev/null 2>&1; then
  echo "Updating subscription $NEW_MESSAGE_NOTIFICATIONS_SUBSCRIPTION_ID."
  ...
  echo "Creating subscription $NEW_MESSAGE_NOTIFICATIONS_SUBSCRIPTION_ID."
...
if gcloud pubsub subscriptions describe "$CONVERSATION_LIFECYCLE_NOTIFICATIONS_SUBSCRIPTION_ID" >/dev/null 2>&1; then
  echo "Updating subscription $CONVERSATION_LIFECYCLE_NOTIFICATIONS_SUBSCRIPTION_ID."
  ...
  echo "Creating subscription $CONVERSATION_LIFECYCLE_NOTIFICATIONS_SUBSCRIPTION_ID."
...
if gcloud pubsub subscriptions describe "$NEW_RECOGNITION_RESULT_NOTIFICATION_SUBSCRIPTION_ID" >/dev/null 2>&1; then
  echo "Updating subscription $NEW_RECOGNITION_RESULT_NOTIFICATION_SUBSCRIPTION_ID."
  ...
  echo "Creating subscription $NEW_RECOGNITION_RESULT_NOTIFICATION_SUBSCRIPTION_ID."
...
ui_connector_service_url=$(gcloud run services describe $CONNECTOR_SERVICE_NAME --platform managed --region $SERVICE_REGION --format 'value(status.url)')
echo $ui_connector_service_url
